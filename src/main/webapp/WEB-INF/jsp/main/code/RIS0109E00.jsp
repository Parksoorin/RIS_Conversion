...
<body>
  ...
  <div class="inquiry__page-flex mt5">
      ...
      <div class="search__container" style="justify-content:space-between;"> <!-- 1 Row -->
        <div class="box__flex">
          ...
          <select id="selectBox" name="selectBox" onchange="searchBox(this.value)">
            <option value="" selected="selected">대분류 코드를 선택해주세요.</option>
          </select>
    ...
    <form commandname="VO" id="savefrm" name="savefrm" method="post">
      ...
      <input type="hidden" name="lrgc_cd_value" id="lrgc_cd_value" value="">
...
<script type="text/javaScript" language="javascript">
  ...
  function lrgc_cd_list(){
    $.ajax({
      type : "post",
      url : "/RIS0109E00CodeList.do?checkLMS=L",
      dataType : "json",
      ...
      success : function(data) {
        ...
        if(data.rows.length  < 1){
          ...
          for(var i = 0; i <data.rows.length ; i++){
            $("#selectBox").append("<option value='"+data.rows[i].lrgcCd+"'>"+data.rows[i].lrgcKrNm+"</option>");
            lrgc_text += data.rows[i].lrgcCd+":"+data.rows[i].lrgcKrNm+";"
          }
        }
      }
    });
  ...
  function clmn_appl_dvsn(){
    $.ajax({
      type : "post",
      url : "/RIS0109E00CodeList2.do?checkLMS=M&hsptId=A001&lrgcCd=CLMN_APPL_DVSN",
      dataType : "json",
      contentType : "application/json; charset=UTF-8",
      async : false,
      error : function(){
      ...
      success : function(data) {
        ...
        for(var i = 0; i <data.rows.length ; i++){
          clmn_text += data.rows[i].mddlCd+":"+data.rows[i].mddlKrNm+";"
        }
      }
    });
  }
  ...
  $(document).ready(function(){
    ...
    $('#list').jqGrid({
      ...
      colModel:[
        ...
        { name: 'clmnNm'	  		, index: 'clmnNm'			, width: '100px' , align:"left" }																	,
        { name: 'applKrNm' 		, index: 'applKrNm' 		, width: '100px' , align:"left" , editable: true , editoptions:{maxlength : 100}}					,
        { name: 'applEnglNm' 	, index: 'applEnglNm' 	, width: '100px' , align:"left" , editable: true , editoptions:{maxlength : 100}}					,
        { name: 'clmnApplDvsn'	, index: 'clmnApplDvsn' 	, width: '80px' , align:"left"  , editable: true ,
          formatter:"select",
          ...
          editoptions: {dataUrl:'', buildSelect:function(){
              ...
              for(var i=0;i<clmn_Appl_Dvsn.length;i++){
                s += '<option value="' + clmn_Appl_Dvsn[i].mddlCd + '">' + clmn_Appl_Dvsn[i].mddlKrNm+ '</option>';
              }
              return s + "</select>";
            }  }
        }		,
        { name: 'rqrdYn' 			, index: 'rqrdYn' 			, width: '50px' , align:"center"  , edittype:"checkbox",editable:true, editoptions: { value:"Y:N" }},
        { name: 'elctLrgcCd' 	, index: 'elctLrgcCd' 	, width: '100px' , align:"left" , editable: true ,
          formatter:"select",
          ...
          editoptions: {dataUrl:'', buildSelect:function(){
              ...
              for(var i=0;i<lrgc_Cd_List.length;i++){
                s += '<option value="' + lrgc_Cd_List[i].lrgcCd + '">' + lrgc_Cd_List[i].lrgcKrNm+ '</option>';
              }
              return s + "</select>";
            }  }
        }		,
        { name: 'iputNm1' 		, index: 'iputNm1' 		, width: '50px' , align:"left"  , editable: true , editoptions:{maxlength : 100}},
        { name: 'iputVlue1' 		, index: 'iputVlue1' 		, width: '50px' , align:"left"  , editable: true , editoptions:{maxlength : 100}},
        { name: 'iputNm2' 		, index: 'iputNm2' 		, width: '50px' , align:"left"  , editable: true , editoptions:{maxlength : 100}},
        { name: 'iputVlue2' 		, index: 'iputVlue2' 		, width: '50px' , align:"left"  , editable: true , editoptions:{maxlength : 100}},
        { name: 'iputNm3' 		, index: 'iputNm3' 		, width: '50px' , align:"left"  , editable: true , editoptions:{maxlength : 100}},
        { name: 'iputVlue3' 		, index: 'iputVlue3'		, width: '50px' , align:"left"  , editable: true , editoptions:{maxlength : 100}},
        { name: 'iputNm4' 		, index: 'iputNm4' 		, width: '50px' , align:"left"  , editable: true , editoptions:{maxlength : 100}},
        { name: 'iputVlue4' 		, index: 'iputVlue4' 		, width: '50px' , align:"left"  , editable: true , editoptions:{maxlength : 100}},
        { name: 'iputNm5' 		, index: 'iputNm5' 		, width: '50px' , align:"left"  , editable: true , editoptions:{maxlength : 100}},
        { name: 'iputVlue5' 		, index: 'iputVlue5' 		, width: '50px' , align:"left"  , editable: true , editoptions:{maxlength : 100}},
      ], //서버에서 받은 데이터 설정
      jsonReader: {
  ...
  function fn_add(){
    ...
    var clmn_nm = ['mddl_cd','mddl_kr_nm','mddl_engl_nm','chrc1','chrc2','chrc3','chrc4','chrc5', 'nmbr1', 'nmbr2', 'nmbr3', 'nmbr4', 'nmbr5']
    jQuery('#list').jqGrid('clearGridData');
    for(var i in clmn_nm){
      var rowData ={
        iud:"I"
        ,	lrgcCd : $("#selectBox").val()
        ,	clmnNm :clmn_nm[i]
      };	// 기본값 셋팅
      if(i < 1){
  ...
  function fn_save(){
    ...
    for(var i in ids){
      ...
      if($('#list').getCell(ids[i], "clmnApplDvsn") != "H"){
        if($('#list').getCell(ids[i], "applKrNm") == null || $('#list').getCell(ids[i], "applKrNm") == ""){
// 				alert((parseInt(i)+1)+"행 [적용명]을 입력해주세요.")
        ...
        if($('#list').getCell(ids[i], "clmnApplDvsn") == "S"){
          if($('#list').getCell(ids[i], "elctLrgcCd") == null || $('#list').getCell(ids[i], "elctLrgcCd") == ""){
// 					alert((parseInt(i)+1)+"행 [선택대분류코드]를 선택해주세요.")
        ...
        }else if($('#list').getCell(ids[i], "clmnApplDvsn") == "C" || $('#list').getCell(ids[i], "clmnApplDvsn") == "R"){
          if(($('#list').getCell(ids[i], "iputNm1") == null || $('#list').getCell(ids[i], "iputNm1") == "")
            || ($('#list').getCell(ids[i], "iputVlue1") == null || $('#list').getCell(ids[i], "iputVlue1") == "")){
// 					alert((parseInt(i)+1)+"행 [입력명1], [입력값1]을 입력해주세요.")
    ...
    for(var i in ids){
      var iud = $('#list').getCell(ids[i], "iud");
      var rqrd_yn = $('#list').getCell(ids[i], "rqrdYn");
      if(i < 1){
        rqrd_yn = "Y"
      }
      fn_reset();
      document.savefrm.iud.value       	   	   = iud;
      document.savefrm.lrgc_cd.value       	   = $('#list').getCell(ids[i], "lrgcCd");
      document.savefrm.clmn_nm.value       	   = $('#list').getCell(ids[i], "clmnNm");
      document.savefrm.appl_kr_nm.value          = $('#list').getCell(ids[i], "applKrNm");
      document.savefrm.appl_engl_nm.value        = $('#list').getCell(ids[i], "applEnglNm");
      document.savefrm.clmn_appl_dvsn.value      = $('#list').getCell(ids[i], "clmnApplDvsn");
      document.savefrm.iput_nm1.value       	   = $('#list').getCell(ids[i], "iputNm1");
      document.savefrm.iput_nm2.value       	   = $('#list').getCell(ids[i], "iputNm2");
      document.savefrm.iput_nm3.value       	   = $('#list').getCell(ids[i], "iputNm3");
      document.savefrm.iput_nm4.value       	   = $('#list').getCell(ids[i], "iputNm4");
      document.savefrm.iput_nm5.value       	   = $('#list').getCell(ids[i], "iputNm5");
      document.savefrm.iput_vlue1.value          = $('#list').getCell(ids[i], "iputVlue1");
      document.savefrm.iput_vlue2.value          = $('#list').getCell(ids[i], "iputVlue2");
      document.savefrm.iput_vlue3.value          = $('#list').getCell(ids[i], "iputVlue3");
      document.savefrm.iput_vlue4.value          = $('#list').getCell(ids[i], "iputVlue4");
      document.savefrm.iput_vlue5.value          = $('#list').getCell(ids[i], "iputVlue5");
      document.savefrm.elct_lrgc_cd.value        = $('#list').getCell(ids[i], "elctLrgcCd");
